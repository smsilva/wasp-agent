...
- `@tool` without `requires_confirmation`: the decorator now takes no arguments (`mocks["agno.tools"].tool = lambda fn: fn` in conftest). `requires_confirmation=True` is incompatible with Telegram.
